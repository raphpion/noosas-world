...
const player = {
  ...
  draw: () => {
    ...
    if (localStorage.getItem('debugMode') == 'true') {
      ...
      for (let i = 0; i < player.hitbox.length; i++) {
        // on affiche les hitbox à leur position relative à la map
        ctx.fillRect(
  ...
  handlePlatforms: () => {
    ...
    for (let platform of game.map.platforms) {
      ...
      if (player.pos.y + player.sprite.height >= platform.pos.y - player.velocity.y && player.pos.y + player.sprite.height <= platform.pos.y + 2) {
  ...
  move: () => {
    ...
    if (keys.shift) {
      ...
      // si la touche a ou d est appuyée, le joueur est en train de marcher et on applique sa vélocité
    ...
    // ou si le joueur va vers la droite et qu'il est dans la zone de défilement droite
    if (
      ...
      // on modifie le offset X de la map
