...
const map_default = {
  ...
  moveEnemies: () => {
    ...
    for (let i = 0; i < map_default.enemies.length; i++) {
      ...
      if (map_default.enemies[i].sprite.direction == 'left') map_default.enemies[i].pos.x -= map_default.enemies[i].speed;

      // si l'ennemi regarde à droite, on le déplace vers la droite
      if (map_default.enemies[i].sprite.direction == 'right') map_default.enemies[i].pos.x += map_default.enemies[i].speed;

      // si l'ennemi n'est plus sur la map, on le retire du tableau des ennemis
      if (map_default.enemies[i].pos.x < 0 || map_default.enemies[i].pos.x > map_default.width) map_default.enemies.splice(i, 1);
    }
  },
  moveKibble: () => {
